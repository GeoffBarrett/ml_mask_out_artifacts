...
def read_data(filename):
    with open(filename, 'rb') as f:
        ...
        with contextlib.closing(mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ)) as m:
            ...
            data_stride = 240  # offset from the t_amplifier
            if header['num_amplifier_channels'] > 0:
                data_stride += 60 * header['num_amplifier_channels'] * 2  # 60 values x X channels x 2 bytes

            if header['num_aux_input_channels'] > 0:
                data_stride += 15 * header['num_aux_input_channels'] * 2
            ...
            if header['num_board_adc_channels'] > 0:
                data_stride += 60 * header['num_board_adc_channels'] * 2
            ...
            if data_present:
                ...
                if (header['version'][0] == 1 and header['version'][1] >= 2) or (header['version'][0] > 1):
                    data['t_amplifier'] = np.ndarray((num_data_blocks,), ('<i', (1, 60)), m, header_length,
                                                     (data_stride,)).flatten()
                ...
                data_values = np.ndarray((num_data_blocks,),
                                         (np.uint16, (header['num_amplifier_channels'], 60)),
...
def get_offset(header, header_length, output):
    if output == 'amplifier_data':
        offset = header_length + 240

    elif output == 'board_dig_in_raw':
        offset = header_length + 240 + 60 * header['num_amplifier_channels'] * 2 + \
                 15 * header['num_aux_input_channels'] * 2 + \
                 header['num_supply_voltage_channels'] * 2 + \
                 header['num_temp_sensor_channels'] * 2 + 60 * header['num_board_adc_channels'] * 2
